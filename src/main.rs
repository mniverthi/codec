...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !compare_vecs(
        &image_bytes[0..PNG_HEADER_OFFSET as usize],
    ...
    while idx < size {
        let chunk_length =
            chunk::combine_bytes(&image_bytes[idx..(idx + chunk::CHUNK_LENGTH_OFFSET as usize)]);
        idx += chunk::CHUNK_LENGTH_OFFSET as usize;

        let chunk_type =
            std::str::from_utf8(&image_bytes[idx..(idx + chunk::CHUNK_TYPE_OFFSET as usize)])
                .unwrap();
